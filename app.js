...
app.post('/log', (req, res) => {
  ...
  if (!message || !username || !req.session.user) {
    return res.status(400).json({ success: false, message: 'Message and valid session required' });
  }
...
//     while (true) {
//         try {
//             const data = await fs.readFile('messages1.log', 'utf8');
//             return res.json({ messages: data.split('\n').filter(line => line.trim() !== '') });
//         } catch (err) {
//             console.log('Retrying to read messages1.log...');
...
app.post('/log', (req, res) => {
  ...
  if (!message || !username || !req.session.user) {
    return res.status(400).json({ success: false, message: 'Message, username, and valid session required' });
  }
...
app.get('/get-processed-messages', (req, res) => {
  ...
  try {
    ...
    res.json({ success: false, messages: [], message: 'Error reading messages' });
  }
});
